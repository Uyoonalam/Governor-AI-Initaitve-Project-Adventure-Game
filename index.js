...
function getRandomIntAttack(min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min;
}
const attackRange = [5, 12];
function getRandomIntBlock(min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min;
}
const blockRange = [0.5, 0.8];
function getRandomIntHeal(min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min;
}
...
class Player {
    HP = 100;
    Attack = [getRandomIntAttack(5, 12), getRandomIntAttack(5, 12)];
    Block = getRandomIntBlock(.8, 1);
    Heal = [getRandomIntHeal(7, 14), getRandomIntHeal(7, 14)];
    Surrender = false;
}
let bossDefeated = 0;
async function play() {
...
class Boss1 {
    HP = 100;
    Attack = [getRandomIntAttack(5, 10), getRandomIntAttack(5, 10)];
    Abilities = this.boss1Heal;
    boss1Heal(player) {
        if (Math.random() < 1 / 3) {
        ...
        switch (playerAction.action) {
            ...
            case "Block":
                ...
                if (blockChance < player.Block) {
                    ...
                    await delay(1000);
                    console.log(`Boss attacks for ${bossAttack} damage, you block ${blockedDamage} damage!`);
